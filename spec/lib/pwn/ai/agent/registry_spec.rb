...
describe PWN::AI::Agent::Registry do
  ...
  it 'CORE_TOOLS is recall-then-act including skills_recall before pwn_eval' do
    ...
    expect(described_class::CORE_TOOLS).to eq(
      %w[memory_recall session_recall skills_recall pwn_eval shell mistakes_record mistakes_resolve learning_note_outcome memory_remember skills_update artifact_read artifact_grep job_run job_status job_tail job_result job_kill]
    )
