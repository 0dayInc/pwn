...
- Catalog: [CWE-689](https://cwe.mitre.org/data/definitions/689.html)
- Abstraction: Compound · Structure: Composite · Status: Draft
...
Do not mark CWE-689 complete because a scanner listed it. Complete means every in-scope instance was attacked or ruled out with saved evidence.

## Tooling (PWN)

PWN::Plugins::ThreadPool, concurrent pwn_eval / shell probes; PWN::Bounty::LifecycleAuthzReplay (two-role replay), PWN::Plugins::BurpSuite

## Procedure

1. **Applicability gate.** Read platforms and introduction phases. If the target has none of them, record a dated negative (`CWE-689 N/A: <reason>`) and stop. Otherwise continue.
...
7. **Adjacent CWEs.** Run the related IDs below (parents first, then peers). Stopping at CWE-689 while a ChildOf parent is also present is incomplete.
8. **Report.** Every finding cites `CWE-689`, instance, oracle, evidence path, and residual risk. A narrative without an artifact is not a test.
