# CWE-557: Concurrency Issues

- Type: Category
- Status/Type field: Draft
- Catalog: [CWE-557](https://cwe.mitre.org/data/definitions/557.html)
- CWE list: 4.20

## What this ID is

Weaknesses in this category are related to concurrent use of shared resources.

## Exhaustive test law

Do **not** file "CWE-557" as a vulnerability. Walk every member. For each member ID, open `references/CWE-<id>.md` and run that procedure. Exhaustion = every member tested or ruled out with evidence.

If a member is Deprecated, still map it to its replacement and test the replacement.

## Members to test

- [CWE-364](CWE-364.md)
- [CWE-366](CWE-366.md)
- [CWE-367](CWE-367.md)
- [CWE-368](CWE-368.md)
- [CWE-386](CWE-386.md)
- [CWE-421](CWE-421.md)
- [CWE-663](CWE-663.md)
- [CWE-820](CWE-820.md)
- [CWE-821](CWE-821.md)
- [CWE-1058](CWE-1058.md)
- [CWE-1322](CWE-1322.md)

## Procedure

1. Copy the member list into the engagement notes.
2. For each member, load `references/CWE-<id>.md`.
3. Run that file's procedure against the target.
4. Tick the member off only when its Verification checklist is done.
5. Summarize coverage: tested / N/A / blocked, with evidence paths.

## Verification

- [ ] Member list captured
- [ ] Every member has a result (tested, N/A, or blocked)
- [ ] No finding uses CWE-557 as the only ID when a member ID applies
