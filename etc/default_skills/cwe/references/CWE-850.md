...
- Catalog: [CWE-850](https://cwe.mitre.org/data/definitions/850.html)
- CWE list: 4.20
...
Do **not** file "CWE-850" as a vulnerability. Walk every member. For each member ID, open `references/CWE-<id>.md` and run that procedure. Exhaustion = every member tested or ruled out with evidence.
...
- [ ] No finding uses CWE-850 as the only ID when a member ID applies
