...
- Catalog: [CWE-897](https://cwe.mitre.org/data/definitions/897.html)
- CWE list: 4.20
...
Do **not** file "CWE-897" as a vulnerability. Walk every member. For each member ID, open `references/CWE-<id>.md` and run that procedure. Exhaustion = every member tested or ruled out with evidence.

If a member is Deprecated, still map it to its replacement and test the replacement.

## Members to test

- [CWE-1002](CWE-1002.md)
...
- [ ] No finding uses CWE-897 as the only ID when a member ID applies
