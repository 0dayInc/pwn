...
- Catalog: [CAPEC-304](https://capec.mitre.org/data/definitions/304.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-304 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-304 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-304`, related CWE IDs, instance, oracle, evidence path.

## Execution flow (minimum techniques)

- Step 1 (Experiment): An adversary sends TCP packets with no flags set and that are not associated with an existing connection to target ports.
...
- [ ] Finding (if any) cites CAPEC-304 and CWE IDs
