...
- Catalog: [CAPEC-106](https://capec.mitre.org/data/definitions/106.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-106 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-106 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-106`, related CWE IDs, instance, oracle, evidence path.
...
- [ ] Finding (if any) cites CAPEC-106 and CWE IDs
