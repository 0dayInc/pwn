...
- Catalog: [CAPEC-391](https://capec.mitre.org/data/definitions/391.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-391 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-391 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-391`, related CWE IDs, instance, oracle, evidence path.
...
- ChildOf → [CAPEC-390](CAPEC-390.md)
...
- [ ] Finding (if any) cites CAPEC-391 and CWE IDs
