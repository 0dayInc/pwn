...
- Catalog: [CAPEC-392](https://capec.mitre.org/data/definitions/392.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-392 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-392 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-392`, related CWE IDs, instance, oracle, evidence path.
...
- ChildOf → [CAPEC-391](CAPEC-391.md)
...
- [ ] Finding (if any) cites CAPEC-392 and CWE IDs
