...
- Catalog: [CAPEC-531](https://capec.mitre.org/data/definitions/531.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-531 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-531 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-531`, related CWE IDs, instance, oracle, evidence path.
...
- [ ] Finding (if any) cites CAPEC-531 and CWE IDs
