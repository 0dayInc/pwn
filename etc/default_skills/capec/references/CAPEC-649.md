...
- Catalog: [CAPEC-649](https://capec.mitre.org/data/definitions/649.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-649 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-649 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-649`, related CWE IDs, instance, oracle, evidence path.
...
- [CWE-46](../cwe/references/CWE-46.md) — run that CWE procedure after this CAPEC flow
...
- [ ] Finding (if any) cites CAPEC-649 and CWE IDs
