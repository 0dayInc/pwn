...
- Catalog: [CAPEC-114](https://capec.mitre.org/data/definitions/114.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-114 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-114 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-114`, related CWE IDs, instance, oracle, evidence path.
...
- [CWE-1244](../cwe/references/CWE-1244.md) — run that CWE procedure after this CAPEC flow
...
- [ ] Finding (if any) cites CAPEC-114 and CWE IDs
