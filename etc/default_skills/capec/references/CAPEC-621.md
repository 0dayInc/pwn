...
- Catalog: [CAPEC-621](https://capec.mitre.org/data/definitions/621.html)
...
A scanner hit or a single blocked request is inventory, not a CAPEC-621 test. Complete means every in-scope instance finished the execution flow (or was ruled out with saved evidence) **and** every related CWE procedure was run or dated N/A.
...
1. **Applicability gate.** Check prerequisites and skills below. If the target cannot satisfy them, record `CAPEC-621 N/A: <reason>` and stop.
...
8. **Report.** Cite `CAPEC-621`, related CWE IDs, instance, oracle, evidence path.
...
- [ ] Finding (if any) cites CAPEC-621 and CWE IDs
