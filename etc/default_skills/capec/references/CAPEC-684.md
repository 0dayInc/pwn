...
- Catalog: [CAPEC-684](https://capec.mitre.org/data/definitions/684.html)
- CAPEC list: 3.9
...
Do **not** file "CAPEC-684" as the only finding ID. Walk every member. For each member, open `references/CAPEC-<id>.md` and run that procedure. Exhaustion = every member tested or ruled out with evidence.

## Members to test

- [CAPEC-447](CAPEC-447.md)
...
- [ ] No finding uses CAPEC-684 as the only ID when a member ID applies
